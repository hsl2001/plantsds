...
def sim_run_segtrace(fasta_path, true_pairs, true_intervals, threads=8, kmer=15):
    """Runs Segtrace with subclustering."""
    out_prefix = "sim_out"
    ...
    return evaluate_caller_output('Segtrace', f"{out_prefix}.dup.bed", true_pairs, true_intervals, t_elapsed)
...
def sim_run_sedef(fasta_path, true_pairs, true_intervals, threads=8):
    ...
    try:
        ...
        if os.path.exists(final_bed):
            return evaluate_caller_output('SEDEF', final_bed, true_pairs, true_intervals, t_elapsed)
...
def sim_run_biser(fasta_path, true_pairs, true_intervals, threads=8):
    ...
    try:
        ...
        if os.path.exists(biser_out):
            return evaluate_caller_output('BISER', biser_out, true_pairs, true_intervals, t_elapsed)
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Simulation benchmark for Segtrace, SEDEF, and BISER.")
    ...
    parser.add_argument("--reps", "-N", type=int, default=5, help="Replicates per genome size (default: 3)")
    ...
    parser.add_argument("--kmer", "-k", type=int, default=15, help="K-mer size for Segtrace (default: 15)")
    ...
    for g_size in genome_sizes:
        ...
        for rep in range(1, reps + 1):
            ...
            # 1. Segtrace
            st_res = sim_run_segtrace(fasta_path, true_pairs, true_intervals, threads=args.threads, kmer=args.kmer)
