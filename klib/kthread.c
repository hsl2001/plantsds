...
void kt_for(int n_threads, void (*func)(void *, long, int), void *data,
            long n) {
  if (n_threads > 1) {
    int i;
    kt_for_t t;
...
static void *kt_fp_worker(void *data) {
  ...
  for (;;) {
    ...
    w->action = 0;
    while (w->action == 0)
      pthread_cond_wait(&fp->cv_s, &fp->mutex);
    action = w->action;
...
void *kt_forpool_init(int n_threads) {
  ...
  for (i = 0; i < fp->n_threads; ++i)
    pthread_create(&fp->tid[i], 0, kt_fp_worker, &fp->w[i]);
  pthread_mutex_lock(&fp->mutex);
  while (fp->n_pending)
...
void kt_forpool(void *_fp, void (*func)(void *, long, int), void *data,
                long n) {
  kt_forpool_t *fp = (kt_forpool_t *)_fp;
  long i;
...
static void *ktp_worker(void *data) {
  ...
  while (w->step < p->n_steps) {
    ...
    for (;;) {
      ...
      if (i == p->n_workers)
        break; // no workers with smaller indices are doing w->step or the
               // previous steps
    ...
    // working on w->step
    w->data =
        p->func(p->shared, w->step,
                w->step ? w->data : 0); // for the first step, input is NULL

    // update step and let other workers know
    pthread_mutex_lock(&p->mutex);
    w->step = w->step == p->n_steps - 1 || w->data ? (w->step + 1) % p->n_steps
                                                   : p->n_steps;
    if (w->step == 0)
      w->index = p->index++;
